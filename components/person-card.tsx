...
export default function PersonCard({
  ...
  return (
    <>
      <motion.div
        className={`relative bg-white dark:bg-slate-800 rounded-xl p-2.5 flex flex-col items-center justify-center transition-all duration-300 cursor-pointer ${getBorderClass()} backdrop-blur-sm bg-opacity-90 dark:bg-opacity-90 hover:shadow-lg max-w-[120px] sm:max-w-[150px] w-full`}
      ...
      >
        ...
        <div className="absolute top-1 right-1 z-10">
          <button
            className="w-5 h-5 rounded-full bg-blue-500/90 hover:bg-blue-600 flex items-center justify-center text-white shadow-md transform hover:scale-110 transition-all"
        ...
        {isAdmin && (
          <div className="absolute top-1 left-1 z-10">
            <button
              className="w-5 h-5 rounded-full bg-slate-500/90 hover:bg-slate-600 flex items-center justify-center text-white shadow-md transform hover:scale-110 transition-all"
              onClick={(e) => {
                e.stopPropagation()
                // Close all other dropdowns by dispatching a custom event
                if (!showDropdown) {
            ...
            {showDropdown && (
              <div
                className="absolute top-6 left-0 bg-white dark:bg-slate-800 rounded-md shadow-lg p-1 min-w-[100px] z-20"
        ...
        <div
          className={`w-12 h-12 sm:w-14 sm:h-14 rounded-full ${getAvatarGradient()} flex items-center justify-center mb-1.5 shadow-lg`}
        >
          <User className="text-white" size={24} />
        </div>

        {/* Person name with truncation */}
        <h3 className="text-sm sm:text-base font-bold text-center text-slate-800 dark:text-white mb-0.5 line-clamp-1 w-full">
          {person.name}
        </h3>

        {/* Age display */}
        <p className="text-xs text-slate-500 dark:text-slate-400 text-center">{age} سنة</p>
        ...
        {person.maritalStatus === "married" && (
          <div className="mt-1 flex items-center justify-center">
            ...
            >
              <Ring size={8} className="inline" />
              {person.gender === "male" ? "اسم الزوجة: " : "اسم الزوج: "}
              {person.spouseName || getMaritalStatus()}
            </span>
          </div>
        )}
        ...
        {/* Children indicator */}
        {hasChildren && (
          <div className="absolute bottom-1 right-1 text-emerald-500 dark:text-emerald-400 bg-emerald-50 dark:bg-emerald-900/30 p-0.5 rounded-full">
