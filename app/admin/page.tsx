...
export default function AdminPage() {
  ...
  const handleAddSubmit = async (formData: FormData) => {
    ...
    const { data } = await supabase.from('people_qaid').select('*')
    setFamilyTree(buildFamilyTree(data.map((p: any) => ({
      id: p.id,
      name: p.name,
      birthYear: p.birth_year,
  ...
  // عمليات CRUD مباشرة على جدول people_qaid
  const addPerson = async (formData: FormData) => {
    const { error } = await supabase.from('people_qaid').insert([
      {
        name: formData.name,
  ...
  const updatePersonDb = async (id: string, formData: FormData) => {
    const { error } = await supabase.from('people_qaid').update({
      name: formData.name,
      birth_year: formData.birthYear,
      death_year: formData.deathYear,
      gender: formData.gender,
  ...
  const deletePersonDb = async (id: string) => {
    ...
    const { data: allPeople } = await supabase.from('people_qaid').select('*')
    ...
    const { error } = await supabase.from('people_qaid').delete().in('id', idsToDelete)
    if (error) throw error
  }
